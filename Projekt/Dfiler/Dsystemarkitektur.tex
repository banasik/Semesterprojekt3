...
\subsubsection{Forstærkning}
For at få den rette forstærkning er det blevet valgt, at benytte operationsforstærkeren INA-114. Her kan transduceren sættes på med det differentierede signal. Under opbygning og modultestning vil det differentierede signal blive simuleret af Analog Discovery. For at udregne den korrekte forstærkning, bruges følsomheden fra transduceren og eksistationsspændingen.
...
\end{split}
\end{equation}
For at få den rette forstærkning udregnes den eksterne modstand ($ R_g $) til INA114 \cite{INA}. INA114's forstærkning kommer af størrelsen på $ R_g $, hvis modstanden er stor, er forstærkningen lille og omvendt.  $ R_g $ udregnes ved formlen: 
\begin{equation}
\begin{split}
...
\end{equation}
Da det bliver oplyst at $ R1=R2 $, kan funktionen reduceres. I sidste ende fås funktionen, se Bilag for nærmere udregninger: 
\begin{equation}
...
\end{split}
\end{equation}
Dette betyder, at kondensatoren $ C1 $ har værdien $ 333\cdot 10^{-9} nF $  og  $ C2 = 680\cdot 10^{-9} nF$. Derved er alle komponentværdierne til lavpasfilteret fundet og det kan nu realiseres. \\ Under udviklingen af lavpas filteret er komponent størrelserne, blevet ændret for at kunne realisere det. De brugte komponent størrelser er: $ R= 6.6 k\Omega $, $ C1= 330\cdot 10 ^{-9} nF$ og $ C2= 680\cdot 10^{-9} nF$.   
...
\subsubsection{Forstærkning}
For at teste forstærkningen sendes et differentieret signal ind, hvor der ses på hvor meget signalet bliver forstærket. 
...
Ved knækfrekvensen skal fasedrejningen være 90\textdegree. Dette kan aflæses på billedet Måling for 50 Hz.
Efter knækfrekvensen skal amplituden gå mod nul. På Måling for 60 Hz, kan det ses hvordan amplituden er faldet drastisk efter knækfrekvensen. 
\begin{figure}[H]
	\centering
...
Efter forstærkning og lavpasfilteret er blevet testet hver for sig, udføres en kalibrering af systemet vha. en vandsøjle. Her bruges en udleveret vandsøjle med tre målepunkter, hvor det er angivet hvor højt trykket er ved hvert af disse punkter, målt i milimeter kviksølv (mmHg). Derved kan det testes om hareware-delen måler den rigtige spænding i forhold til mmHg. Ud fra det maksimale antal volt (V) spænding og mmHg kan det udregnes, hvad hardware skal vise ved 100 mmHg. 
\begin{figure}[H]
	\centering
